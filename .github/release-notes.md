...
|  |  |
|---|---|
| **Root** | [KernelSU-Next](https://github.com/KernelSU-Next/KernelSU-Next) - kernel-side, no ramdisk patch needed |
...
### A note on this variant

KernelSU-Next consumes the SUSFS kernel API directly, so a build only happens
when the driver and the SUSFS revision for that month agree. Months where they
do not are skipped rather than shipped broken.
